...
class ContentBlock(dict):
    def __init__(
        ...
        Args:
            ...
            image_type (str or None): The image type for saving (e.g., '模糊图片', '残缺图片', '错误识别').
        """
        super().__init__()
    ...
    def image_type(self) -> Optional[str]:
        return self.get("image_type")

    @image_type.setter
    def image_type(self, value: Optional[str]):
        if value is not None:
            self["image_type"] = value
        elif "image_type" in self:
            del self["image_type"]
